...
export default defineComponent({
    ...
    computed: {
        ...mapState(useMessageStore, ['successMessages']),

        hasSuccess() {
            return Object.keys(this.successMessages).length > 0;
        }
    },

    methods: {
        hideMessage() {
            useMessageStore().successMessages = {};
            this.doShow = false;
        }
    },
...
<template>
    <Transition name="msgDisplay">
        <div v-if="hasSuccess === true && doShow === true"
             class="card__message card__success text-center">
            ...
            <div class="div__msgs">
                <p v-for="(success, index) in successMessages" :key="index">{{ success }}</p>
            </div>
        </div>
    </Transition>
</template>
